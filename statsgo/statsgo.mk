...
db:: db/statsgo db/statsgo.map_unit_poly db/statsto.map_unit
db/statsgo:
	${PG} -f statsgo.sql
	touch $@
...
db/statsgo.map_unit_poly: db/statsgo
	[[ -f ${shp} ]] || unzip -j ${zip};
	${shp2pgsql} -d -r 4269 -g boundary -s ${srid} -S ${shp} statsgo.map_unit_poly | sed -e 's/, ${srid}));$$/::geometry, ${srid}));/' | ${PG} > /dev/null;
	db/statsgo.map_unit_poly
...
db/statsgo.comp:db/statsgo.%:db/statsgo.mapunit
	${PG} -f $*.sql
	unzip -p ${zip} ${tab}/$*.txt |\
	${PG} -c "COPY statsgo.$* (${$*.cols}) FROM STDIN DELIMITER AS '|' CSV QUOTE AS '\"'";
	touch $@

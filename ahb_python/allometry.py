...
#    '''
#    from sympy.abc import l,b,t,V
#    eqn=(l*sy_pi/12)*(b**2+t**2+b*t)-V
...
def getDBH(stem_mass, coeffs='dbh_coeffs.csv', spp=['Acer saccharum', 'Populus tremuloides']):
    ...
    try:
        ...
        f.close()
    return pow(stem_mass/a, float(1/n))
...
def brahAllo(solve_for='H'):
    '''
    solve allometric equation from Brahim2000 for
    H= height
    D= diameter at breast height
    p= stand sverage diameter at breast height
    W= dry weight
    ...
    from sympy.abc import W,H,D,p
    a,b,c,d=3.85, 68E-06, 2.34, -0.017
    eqn=solve((a+b*H*pow(D,c)+d*pow(p,c))-W,solve_for)
    return [eqn, latex(eqn)]
...
def estBd(dbh,height,coeffs):
    ...
    H=height/100
    d=dbh
    eq=solve((B-B*(log(1-h/H*a,10)/-b)**1/c)-d,B)[0]
    m=eq.evalf()
    return m.evalf()   
